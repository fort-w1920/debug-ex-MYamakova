...
match.arg <- function(arg, choices, several.ok = FALSE) {
  ...
  if (missing(choices)) {
    ...
    # evaluation frames hat (also: Nummer der Funktion im *call stack* die
    # match.arg() aufgerufen hat)
    # sys.function(<nummer>) gibt die Funktionsdefinition zurück die an der
    # <nummer>-sten Stelle im call stack steht -- deren "formals" lesen wir aus:
    formal.args <- formals(sys.function(sysP <- sys.parent()))
    # choices = mögliche optionen für <arg> in der match.arg aufrufenden Funktion.
    # as.character(substitute(arg)) ersetzt (s. ?substitute) zuerst <arg> durch den
    # Ausdruck der an <arg> übergeben wurde ohne diesen auszuwerten, und wandelt
    # diesen dann in einen character-string um damit man damit auf
    # das entsprechende Listenelement in <formal.args> zugreifen kann.
    # das wird dann noch mal mit "eval" in dem execution frame der aufrufenden
    # funktion ("sys.frame(sysP)") ausgewertet, falls dieses listenelement ein
    # nur dort definiertes symbol ist und kein expliziter vektor von alternativen. 
    choices <- eval(formal.args[[as.character(substitute(arg))]],
                    envir = sys.frame(sysP))
  }
  ...
  if (!several.ok) {
    # wenn weder <choices> im Aufruf von match.arg, noch <arg> im Aufruf der 
    # Funktion die match.arg aufgerufen hat, definiert wurden, dann
    # ist <arg> das selbe wie <choices>, da <choices> ja eben die formals für
    # <arg> sind (s.o.) -- also nehmen wir davon das erste und sind fertig
    # (wichtgster default fall!).
    if (identical(arg, choices)) return(arg[1L])
    if (length(arg) > 1L) stop("'arg' must be of length 1")
  } else if (length(arg) == 0L) stop("'arg' must be of length >= 1")
  ...
  if (all(i == 0L)) {
    # gettextf lädt Übersetzung der Fehlermeldung-Strings in die in den
